 ...
 * given fake container. Used by the executed wheel-guard checks, which need
 * the method's internal `document` lookup to find their stub.
 *
 * @param {object} container - Fake element with addEventListener.
 * @returns {object} The same shape loadRuntime() returns.
 */
function loadRuntimeWithContainer(container) {
  const storage = { getItem: () => null, setItem: () => {} };
  const win = { matchMedia: () => ({ matches: false }) };
