...
const standaloneTests = [
  ...
  'terminal-surface.test.js',        // Notion restyle P5.4: the single terminalSurface projection (13 palettes, the 5 mock slots, the 16 ANSI slots), the DRIFT GATE that re-derives every palette from styles.css so a token edit can never desynchronise the terminal from its chrome, the six-digit-hex constraint risk R5 forces, and the contrast floors TERMINAL-ARCHITECTURE 10.5 gates
  ...
  'mobile-ia-contract.test.js',      // Notion restyle P10.1/P10.2: the phone capability manifest, the five-tab order, every data-mw-route marker in both directions, and the regression gate that no capability's only route sits inside the display:none pane header
  'mobile-viewport.test.js',         // Notion restyle P10.3: the visualViewport geometry driver EXECUTED against a fake viewport (the four custom properties, the settle debounce, the keyboard threshold) plus the three source-level bans it replaces
  'mobile-touch-model.test.js',      // Notion restyle P11: the three-zone long-press resolver, the priority-plus toolbar fit and the geometry claim gate, all EXECUTED against fakes, plus the toast, swipe, drag and Sessions-surface contracts as source gates
  'credential-protections-gate.test.js', // task #33: the credential-protection regression gate. The audit found ZERO drift between this branch and the verified integration branch, so this file turns the audit's five findings into standing assertions: the single abort controller spanning the body read, the op deadline and stalled-chain watchdog, degraded list responses, the proactive sweep's default/window/lineage gate, the write-back theft guard, the honest verdict table with the suspect ladder and the six-hour dead retry, and the completeness check that EVERY mutating credential route carries the ownership guard
  'mac-host-migration.test.js',      // task #37: the Mac host default and its single definition, the ordered candidate chain, probeMacHosts as a read-only diagnosis that suggests and never redirects, and the one-time stored-value migration (what it rewrites, what it refuses to touch, passive-mode skip without burning the marker, and that it contacts nothing so a powered-off Mac cannot block it)
  'claude-discover-budget.test.js',  // task #36: the Claude discovery walk stops owning the event loop. Parity against the pre-fix synchronous walk, the (mtimeMs,size) title cache with its invalidation and prune, the explicit slice yield proved by a heartbeat, caller coalescing, and the async readers agreeing with the synchronous ones they were split from
  'terminal-touch-window.test.js',   // Notion restyle P11b (post-P7 mop-up): the claim gate inside _requestActivate, one table of gesture constants, the touch boundary in both directions with its momentum carry-through, B.4's jump pills and the native-selection/pull-to-refresh stylesheet contract, reduced motion asked of the platform, the IntersectionObserver window proved on a 50000-line document (measured heights, freeze-safe, select-all hydration, byte-identical text), and E.3/E.4's caps: 2000-line phone ring, two live panes, three flush cadences, 200k tail-biased Reader
  'pwa-manifest.test.js',            // Notion restyle P12.6 (MOBILE-EXPERIENCE F, BUILD-CONTRACT P12.1): the installable shell asserted against the PNG headers rather than against the JSON, because the bug it exists for was two manifest entries declaring 192 and 512 while both pointed at one 691x361 file. Plus display:standalone, a real maskable with its own safe-zone padding, the manifest ground agreeing with the pre-paint data-chrome, the theme-color PAIR and the querySelectorAll sync that rewrites both, the iOS tags Safari actually reads, the black-translucent deferral kept honest, and the proof that the registered service worker stub can neither cache nor answer a request
  'terminal-auth-recovery.test.js',  // round 1 post-launch: a server restart invalidates every in-memory token, so panes must park on cwm:auth-ready and retry after login instead of dead-ending on "No auth token"; one token source, one prompt, no retry storm against a rejected upgrade
  'codex-detail-trace.test.js',    // round 2: the codex peek/strip showed values no session was running (invented gpt-5-codex/workspace-write/on-request/medium) because the discovery payload dropped the P8 state-db reading; this pins the carry-through, the sandbox_policy struct reduction, override>observed>unset resolution, the UI enum sets against spawn.js, and the argv assembly for every menu value
  'asset-delivery.test.js',          // round 2: a deploy has to reach the phone. The document is served no-store (a shared cache holding one stale index.html pins every ?v= inside it), every local asset is referenced exactly once, and every first-party asset is versioned; vendor bundles are a named exemption
  'terminal-font-coverage.test.js',  // round 1 post-launch: the Codex TUI drew box-drawing frames and braille spinners through a vendored face that maps neither, so the grid tore; the gate MEASURES the woff2 cmaps and requires the terminal chain to lead with a coverage-complete mono
  'provider-filter-scope.test.js',   // round 1 post-launch: the provider switcher lives in the Discovered header and filters that list ONLY (placement, scope table, row-level session filter for a merged folder, discovered-based badge counts with the tracked fallback)
  'terminal-history.test.js',        // Notion restyle P7: the Unified Scrollback Surface EXECUTED against a fake DOM (the buffer-mode router and its sidecar-free fallback, the string-enum mouse-tracking discipline, the scroll boundaries in both directions, the wheel escalation and its off switch, the mirror freeze surviving 200 lines of output, the live screen as the document's LAST segment) plus the seam arithmetic, the bounded history route, and the P7.6 preservation gate that no Select mode identifier was removed
  'brand-assets.test.js',            // feat/brand-vector: the hand-authored vector brand set. The old docs/images/logo-animated.svg was a base64 PNG in an <svg> wrapper with zero paths, and nothing failed; so this reads the files rather than their extensions (no <image>, no base64, no filter, square viewBox, PNG IHDR against the declared size, WebP VP8X/ANIM/ANMF for animated + loop 0 + 2 to 3 s) and compares the four silhouette paths byte for byte across all five files that draw the mark, plus the double-hyphen-in-an-XML-comment gate that silently blanked both lockups during authoring
  'media-pipeline.test.js',           // media pipeline (2026-08-18): the hermetic half of the marketing capture. Fixture invariants (no real path, name or cost can reach a frame; both providers; the retired Codex default SET can never co-occur; buildFixture is pure in `now`), the encode argument builders against the contract's own frame rates, widths and budgets plus the five silent ffmpeg failures, the terminal player's glyph repertoire MEASURED against the vendored woff2 cmap, the capture scene table, and the CWM_CLAUDE_PROJECTS_DIR override in both directions
  'status-marks.test.js',           // 2026-08-18 status-mark sweep (DECISIONS 13.6): the shape half of the standing ban that gate G14 only covered for motion. The two surfaces that shipped a capsule around a dot (.status-badge in the sessions table and the peek, .stat-chip in the header) are asserted de-capsuled rule by rule, the ring is asserted on the two waiting states, the four previously unstyled status dots are asserted to exist, the status ink mix is asserted inside its measured contrast window, and statusChipHtml is lifted out of app.js and EXECUTED so the class names, the eight labels, the escaping and the two fallbacks are proved on the emitted string rather than on the source
  'site.test.js',                   // 2026-08-18 landing site: the only gate over site/, which nothing else in the suite reads. Structure and section ids, the raw.githubusercontent asset contract shared with the README and docs/marketing/MEDIA-CONTRACT.md, no em dash in any site file, gate G14's shape applied to the site stylesheet plus the markup check that no chip encloses a dot, one first-party script, the link-preview and icon tags, width/height/alt on every image, and the Pages workflow triggers, permissions and action majors
  'mobile-terminal-geometry.test.js', // 2026-08-19 shared-PTY geometry (docs/design/notion-restyle/MOBILE-TERMINAL.md): the hermetic half of the fragmentation fix that npm run test:mobile-terminal proves end to end. The input classifier that tells a keystroke from a DEC 1004 focus report (the measured payload that took a live desktop from 155 columns to 49 with nothing pressed), the size frame's shape including "nobody is driving" against "somebody else is", the type-size ladder arithmetic that turns a 390px pane into 63 columns instead of 49, and the source contract: safeFit defers to the published grid before it fits, a follower reports its OWN viewport, ambient claims stand down against a live owner while an explicit take-over never does, the affordance compares against the server rather than against itself, and the follower viewport pans on both axes
];
