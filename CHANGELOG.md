...
- **Terminal output on a phone is readable again.** Opening a session on your phone while your laptop was driving it produced a screen that was, in the report, "broken up and scattered and frayed here and there, badly formatted": lines cut in half with their endings stranded on lines of their own, most of the screen missing, and leftovers from an earlier draw sitting under the rest. The cause is that a terminal session has one size, one device sets it, and nobody told the others what it was. Your phone therefore drew a screen the CLI had painted for a 155 column laptop into its own 49 columns, and because an agent CLI paints by putting text at exact row and column positions rather than by printing lines, every row that had to wrap pushed everything after it onto the wrong row. Measured on the reported case: none of the 30 frame rows survived intact, 20 were left as fragments and 2.9 percent of the laptop's screen was reproduced. Now the server tells every connected device what size the session actually is, and a device that is not setting it draws that size exactly and lets you pan across it, so both screens show the same thing. The same case now reproduces all 30 rows, with no fragments and nothing out of place. The screen you get on a cold reconnect, which is rebuilt from the server rather than from live output, was broken in exactly the same way and is fixed by the same change.
- **Looking at a session on your phone no longer shrinks the terminal on the machine you are working on.** A terminal answers questions the application asks it, including "did you just gain or lose focus", and those replies were indistinguishable from typing, so merely opening a session on a second device claimed the session's size for it. Measured: 155 columns became 49 with no key pressed. Replies from the terminal itself no longer claim the size, and neither does a device simply becoming visible or switching to its Terminal tab while another device is driving. Typing, tapping into the pane, the on-screen keys and the take-over banner all still claim it immediately.
- **The phone now says when another device is setting the width, and offers to take over.** The notice for this had shipped but could never appear: it compared the applied width against the phone's own preferred width, which on a device that always sized itself to its own screen were the same number. It reads the real session size now, names it, and one tap hands the session to the phone.
- **The scrollback view is set in the terminal's own typeface again.** Scrolling back through a session rendered the whole history, live screen and transcript alike, in the app's proportional interface font at 14px, so columns did not line up and box drawing collapsed. It was reading the typeface from the wrong element, and the test that was supposed to catch it was reading from that same wrong element, so it passed while both were wrong.

### Added

- **Terminal type size is yours to choose on a phone.** A- and A+ in the terminal key row, remembered between visits, stepping a fixed ladder from 8px to 16px. A phone that has never been told otherwise now measures its own screen and picks the largest size that fits at least 60 columns, which on a 390px phone is 10px and 63 columns instead of the 13px and 49 columns it used to inherit from the desktop. Changing the size resizes the session only when your phone is the device driving it. At 390px in portrait the two keys live in the key row's overflow sheet, alongside the other keys that do not fit; in landscape and on wider phones they sit in the row itself.
