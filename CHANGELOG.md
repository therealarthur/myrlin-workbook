...
- **Ctrl+C to copy from a terminal pane sent SIGINT to the running CLI on plain-http remote access (user-reported, 2026-07-24).** The Ctrl+C branch of the terminal key handler called the async Clipboard API directly with a chained `.catch()`. On an insecure origin (plain http to a LAN IP or hostname, the README's documented remote-access mode) `navigator.clipboard` is undefined, so the property access threw a SYNCHRONOUS TypeError that `.catch()` could never intercept (it only handles promise rejections, not a throw during property lookup). The exception escaped the handler BEFORE `clearSelection()` and `return false` executed, xterm treated the key as unhandled, and the "copy" keystroke delivered `\x03` (SIGINT) that interrupted the user's running session; the copy itself also silently failed. This is the copy-side twin of paste issue #64, whose fix feature-detected the API on the Ctrl+V branch directly below but was never applied to Ctrl+C. Fixed with a universal helper plus a structural guarantee:
  ...
  - **Tests:** new `test/copy-secure-context-fallback.test.js` (16 checks, registered in `test/run.js`): source gates on the branch shape, the function-level feature detect, the finally cleanup, and a zero-bare-writeText gate over app.js, plus EXECUTED vm proofs that the real extracted branch with `navigator.clipboard` undefined still returns false (key consumed, no SIGINT) while the selection lands on the stub clipboard via the fallback, that cleanup survives an execCommand throw, and that the no-selection fall-through (intentional SIGINT) is preserved. The pre-fix branch was run under the same harness and demonstrably throws before `return false`.
...
- **Select mode v2 for terminal panes: output freezes while you select, plus a Copy view that captures everything.** Select mode v1 (2026-07-25) made a plain left drag behave like a Shift+drag so xterm would force a selection while the running CLI holds mouse tracking. That fixed the gesture but not the content. xterm 6 anchors a selection to ABSOLUTE buffer coordinates and clears it only on user input, scrollback trim, a row-count resize, or a buffer switch; a plain PTY write leaves the selection object intact while repainting the cells underneath it. An interactive CLI repaints width-locked full-screen frames continuously, so a valid selection silently became stale text within a frame or two and the copy came out scrambled. Two additions in `src/web/public/terminal.js`, both frontend-only:
  - **Freeze-while-selecting.** While Select mode is ON, incoming PTY bytes accumulate in the write-batching queue that already existed (`_enqueueWrite`) and no flush is scheduled, so the screen the user is dragging over stops changing and the selection stays exactly what they see. Nothing is dropped: toggling off drains the whole hold in a single `term.write()`. Every path that sends bytes to the session (xterm `onData`, both paste listeners, Ctrl+V, Shift+Enter, `sendCommand`) leaves the mode BEFORE the send, so the held frames land first and the echo of what was just typed renders on a current screen rather than behind stale output. Both resync paths (a WebSocket reconnect and a server-initiated `reset`, which precedes a full scrollback replay) drop the freeze before `term.reset()`, so a replay can never queue up behind a frozen pane. A container resize arriving mid-freeze is deferred rather than applied, because a row-count change is one of the few things xterm treats as a reason to clear the selection outright; the fit runs the moment the freeze lifts, after the drain, so old-width frames are never rendered into a new-width grid. A 2MB cap on held output is the overflow valve: past it the pane leaves Select mode, flushes, and says so in the strip, which is strictly better than unbounded memory growth followed by a multi-second stall.
  - **Wheel guard.** xterm 6 forwards the wheel to the PTY whenever the app has mouse tracking on and offers no Shift bypass for local scrolling, so one wheel notch during a selection would scroll the app and repaint every cell under the highlight. A capture-phase wheel listener on the pane container (inert while the toggle is off) swallows the wheel when the ALTERNATE buffer is active, where there is no scrollback to scroll anyway, and translates it into a local `term.scrollLines()` when the NORMAL buffer is active, so plain shell panes keep scrolling (and drag-at-edge auto-extension keeps working) while selecting.
  - **Copy view overlay.** A second header button opens an in-pane overlay holding a text SNAPSHOT of the pane rendered as ordinary DOM text, which is the honest answer to "copy the whole conversation": an interactive CLI keeps its history inside the app and writes nothing to terminal scrollback, so no amount of terminal selection can reach it. When the alternate buffer is active the snapshot is the FULL normal buffer (the pre-app transcript plus shell scrollback, which stays readable while the alternate buffer is on screen), a `[ current screen ]` divider, then every row of the live frame; otherwise it is the normal buffer including scrollback. Runs of three or more blank lines collapse to one and trailing padding is trimmed, because full-screen frames are mostly empty rows. Native selection, drag-scrolling, long-press selection on touch, Escape, and a Ctrl+A scoped to the overlay all work, keystrokes inside the overlay never reach app-level shortcuts, and "Copy all" routes through the existing universal clipboard helper so it works on plain http LAN origins too. The overlay does NOT freeze the terminal (it is a snapshot; the session keeps streaming underneath), reuses one DOM node across repeated opens, and is torn down with its listeners in `dispose()`. All of its styling is injected from terminal.js against Catppuccin custom properties, so no stylesheet ships with it and every theme flavor follows automatically.
  - **Visible-pane width claim (phone readability).** One PTY is shared by every attached client, and the backend gives geometry ownership to whichever client last typed or sent an explicit `activate` (v1.2.0). Until now only a click reached that path, so a phone that switched to a pane it was already attached to kept rendering frames laid out for a desktop and wrapped them into porridge. A pane now claims ownership itself on two edges: its container crossing 50% visibility (`IntersectionObserver`, hidden-to-visible edge only, so a pane that stays on screen never re-claims) and xterm's hidden textarea taking keyboard focus (tab key, programmatic focus, the mobile Type button, none of which reach the app's click handler). The pane fits before claiming so the viewport the server stores is the one it is about to apply, and the CLI then does a clean full repaint at that width. Guards: at most one claim per 500ms; an identical cols/rows claim is suppressed for 5s and then allowed again because ownership can move to another device at any time; nothing is claimed while Select mode is frozen (the repaint would pour into the hold queue and the fit could clear the selection), where the intent is remembered and replayed on unfreeze; and nothing is claimed inside a 1.5s quiet window after a connect or a server-initiated resync, where the intent becomes a queued retry rather than a tear in the scrollback replay. Teardown disconnects the observer, detaches the focus listener, and cancels a queued retry, so a disposed pane can never claim geometry for a session it no longer shows. The honest limit of a single shared PTY, stated plainly: a desktop watching the SAME session then renders the phone's narrow frames until it is interacted with again, at which point it claims back.
  - **Tests.** New `test/terminal-select-v2.test.js` (56 checks, registered in `test/run.js`): executed proofs that a frozen pane holds bytes and schedules nothing, that toggle-off drains in exactly one write, that the deferred fit runs after the drain, that the real extracted `reset` branch unfreezes before `term.reset()`, that the 2MB valve flushes and notifies, that the real wheel guard is inert when off and swallows or redirects by buffer type, plus snapshot-composition coverage (both buffers with divider, normal-only, blank-run collapsing, unreadable rows) and scoped source gates on the input-path unfreeze ordering and the overlay's text-not-markup, theme-token, and teardown contracts. The width-claim half adds executed coverage of the debounce (two rapid triggers claim once), the identical-geometry suppression and its 5s expiry, the freeze deferral and its replay on unfreeze, the quiet-window retry, single-timer discipline, closed-socket and send-failure paths, and a stub-observer proof that only the hidden-to-visible edge past the ratio claims.
